...
class MafiaAgent:
    ...
    def save(self, candidates: List[str], game_state=None) -> str:
        """Doctor: choose a player to protect from mafia kill tonight"""
        all_players = [a.name for a in game_state.agents]
        other_players = [name for name in all_players if name != self.name]
        random.shuffle(other_players)

        saving_prompt = format_night_action_prompt(
            name=self.name,
            other_players=', '.join(other_players),
            composition=game_state.get_composition_string(),
            memory=self.get_memory(),
            round_num=game_state.round,
            action="protect (save from mafia kill)",
            candidates=', '.join(candidates)
        )

        response = self._generate(saving_prompt, max_tokens=self._get_token_limit('night_action'))

        target = parse_night_action_response(response, candidates)
        if not target:
            target = random.choice(candidates)
            game_state.log_action("save", self.name, response, f"{target} (random)")
        else:
            game_state.log_action("save", self.name, response, target)

        self.remember(f"You protected {target} tonight.")
        return target